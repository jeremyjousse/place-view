...
**Place View** is a multi-platform application for viewing places and webcams, likely focusing on mountain/ski resort areas (inferred from `resources/france-montagne-places.json`).

The project is structured as a monorepo containing:
*   **Web Applications:** Two SvelteKit applications (`webapp`, `admin-tools-svelte`) and one Next.js application (`admin-tools`).
*   **Mobile Application:** An iOS application (`swift-app`) built with SwiftUI.
*   **Data:** Shared JSON resources in the `resources` directory.

## Directory Structure

*   **`admin-tools/`**: Legacy Admin Dashboard.
    *   **Stack:** Next.js 15 RC, React 19 RC, Tailwind CSS, Shadcn UI.
    *   **Status:** Likely being replaced by `admin-tools-svelte`.
*   **`admin-tools-svelte/`**: New Admin Dashboard.
...
### Web Projects (`admin-tools`, `admin-tools-svelte`, `webapp`)
...
**`admin-tools-svelte` (SvelteKit):**
```bash
cd admin-tools-svelte
pnpm dev      # Start development server
pnpm build    # Build for production
